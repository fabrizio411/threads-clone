...
import { createComment, createThread, quoteThread } from '@/libs/actions/threads.actions'
import QuoteThreadCard from '@/components/cards/quote/QuoteThreadCard'
...
const CreateForm: React.FC<CreateFormProps> = ({ parentId, parentAuthor, isComment, username, image, isPrivate, userId, quoteThreadInfo, handleCloseModal }) => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    } else if (quoteThreadInfo && handleCloseModal) {
      setIsLoading(true)
      await quoteThread({
        currentUserId: userId,
        quoteThreadId: quoteThreadInfo.threadId,
        quoteAuthorId: quoteThreadInfo.authorId,
        quoteAuthorUsername: quoteThreadInfo.authorUsername,
        body: data.body,
        image: data.image,
        path: pathname
      })
  ...
  return (
    <form className='create-form-component' onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className={`form-display ${isComment && 'comment'} ${quoteThreadInfo && 'quote'}`}>
        ...
        {quoteThreadInfo && (
          <QuoteThreadCard 
            authorImage={quoteThreadInfo.authorImage} 
            authorUsername={quoteThreadInfo.authorUsername} 
            threadBody={quoteThreadInfo.threadBody} 
            threadImage={quoteThreadInfo.threadImage} 
            threadId={quoteThreadInfo.threadId}
          />
        )}
